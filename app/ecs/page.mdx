...
For the player entity to participate in the new dynamic, the `inputSystem` will stop setting location directly and instead affect velocity, a bit like a strong wind. The new `momentumSystem` will apply velocity to position and friction to velocity. We'll need a couple more components to support this:
...
You might also notice that when systems update components, they're mutating the entities in place, something we avoid in React land. This is a bit faster than creating new entities for every system at every tick, and saves on garbage-collection.
...
It's time to add collision-detection, which will be the basis of all interaction between entities. On my first pass, I thought collision and rebound would be a single system, but not all collisions should rebound. Entering an invisible box might be an important game condition.

I'm used to thinking of collision as an event, like the browser's `mouseEnter`, but a natural way to do it with ECS is to add a temporary `collisionEvent` to the entities involved. Other systems can use these however they need to.

To discover collisions, we'll need to look at the position of every collidable entity in relation to every other collidable entity. I've written a helper function, `forEntsWith`, to make this nested looping quick and type-safe.

The first thing to do is clear any existing collision events from the previous tick. Because systems always run in the same order, we know that all the other systems have had a chance to read it already. Then we look at the new state of play.

We compare the left, right, top, and bottom edges of every entity. If there's overlap on BOTH X and Y axes, we know they're occupying some of the same space, and we can add a collision event. That event will contain the following data:

- `xOverlap`: How much the entities overlap on the X axis.
- `yOverlap`: How much the entities overlap on the Y axis.
- `otherEntity`: The entity we're colliding with (cloned, not referenced). Useful data for rebounds and other interactions.

Below, I've provided a real-time display of the overlap data being stored on the red player-controlled entity, which I've also made transparent.
...
As you scoot around, think about how you'd expect different collision data to effect the entities involved. A large `xOverlap` and a small `yOverlap` suggests a vertical collision. Unless we're modeling friction between the smooth faces of the entities (let's not!), they should trade velocities on the Y axis.

I have to point out here that I can only model this mentally because everything in the game is a rectangle and never rotates. Maybe one day I'll try a more realistic simulation.

Now to write the system that will use these collision events.

export default function MDXPage({ children }) {
  return <MdxLayout>{children}</MdxLayout>;
}
import { forEntsWith } from "./sandpackFiles/4/src/forEntsWith";
import { collisionSystem } from "./sandpackFiles/4/src/collisionSystem.ts";
