...
In addition to the velocity updates, we can use the saved overlap value to instantly place entities safely outside collision bounds, which instantly fixed a lot of bugs.
...
There's also a loop over multiple collisions that might accrue on a single entity. If an entity runs directly into a corner formed by 2 walls, though the collisions may be simultaneous, we can treat them collisions sequentially. Wall-X reverses X velocity, then wall-Y reverses the Y velocity, and the entity flies out in the opposite direction.

It was a lot of work, but now we can bounce other entities around and keep the game contained with walls, which we got practically for free.
...
It's finally time to make the game winnable. Green entities are bouncing around turning other entities green on contact. The objective is to shepherd two pink entities into the yellow safe-zone without turning green yourself.
