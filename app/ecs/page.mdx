...
import systemsTs1 from "!!raw-loader!./sandpackFiles/1/src/systems.ts";
...
import engineTs6 from "!!raw-loader!./sandpackFiles/6/src/engine.ts";
import renderSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/renderSystem.ts";
import inputSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/inputSystem.ts";
import momentumSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/momentumSystem.ts";
import collisionSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/collisionSystem.ts";
import reboundSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/reboundSystem.ts";
import zombieSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/zombieSystem.ts";
import lurchSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/lurchSystem.ts";
import gameOverSystemTs6 from "!!raw-loader!./sandpackFiles/6/src/gameOverSystem.ts";
import utilsTs6 from "!!raw-loader!./sandpackFiles/6/src/utils.ts";
import entitiesTs6 from "!!raw-loader!./sandpackFiles/6/src/entities.ts";
...
That's what I was after when I came across a handful of blog posts about the Entity-Component-System architecture (ECS). First it was the [Starmancer devblog](https://www.playstarmancer.com/devblog/2018/9/25/component-system-same-face-new-engine), which didn't reference ECS directly but gave a feel for the pleasure of a radically extensible game engine. Then I found Maxwell Forbes' magisterial series on [building an ECS in Typescript](https://maxwellforbes.com/posts/typescript-ecs-what/).

Proponents say that ECS manages the complexity of interacting game dynamics by keeping them decoupled. If you're less worried about what breaks when you add or edit a game dynamic, then you're closer to an open-ended, additive process – my ideal of recreational programming.

There are lots of ways to build an ECS, and lots of optimization tricks to make it work at scale. I made mine up as I went along and didn't do any optimization, but I ended up with something pretty fun. I've never made a real-time game before, so I was pleasantly surprised that I could noodle my way to where I ended up.

## The Concept

_Entities_ are just bags of properties. These properties are called _components_, and they store the state of the game. They change over time thanks to _systems_. A system is concerned with one or more specific components. It looks at every entity with those components and makes some changes. At every tick of game time, each system takes its turn processing its relevant entities. I think this is a decent minimal definition of an ECS, but feel free to consider this an engagement gambit: tell me how I'm wrong!

One thing missing from my implementation is a way for systems to efficiently query for entities they should act on, a basic affordance of any real ECS engine. I never encountered any performance issues at this tiny scale, so I couldn't bring myself to obscure the beautiful simplicity of the core loops.
...
options={{
  visibleFiles: ["/src/engine.ts","/src/entities.ts", "/src/systems.ts",],
  activeFile: "/src/engine.ts",
}}
/>

There are three entities above, using different combinations of three components:
...
The `momentumSystem` sees that Red and Green have position and velocity, so it updates their position _by_ their velocity. So simple! Notice how there's not a 1-to-1 mapping between components and systems. Systems generally act as causal links between bits of state.

## Universe Input/Output

The `renderSystem` is an odd duck. It knows about non-gameworld things like the DOM and the canvas API. And we'll need another special system, `inputSystem`, to listen for keyboard events. It feels pretty natural to keep these "impure" systems self-contained, which makes it easier to write tests, or even port the game to another platform, like a Node server.
...
So we need to know the axis of the collision. As you explore collisions above, you'll see that the overlap with the smaller absolute value generally indicates the axis. That stops being true as the overlap becomes more complete, but hopefully our rebound system prevents this from happening! The overlap value also lets us place entities safely outside of collision bounds. Lots of buggy behavior just vanished when I figured this out.
...
There's also a loop over multiple collisions accrued on a single entity, like when an entity runs directly into a corner formed by 2 walls. Though the collisions may be simultaneous, we treat them sequentially: Wall-X reverses X velocity, then wall-Y reverses the Y velocity, and the entity flies back in the opposite direction.
...
It's finally time to make the game winnable. You win if you can shepherd a pink survivor to the yellow safe-zone. You lose if green squares, which are zombies, infect all of the pink squares. Your player square is immune to zombies.
...
- `gameOverSystem`: Detect win/loss conditions and show a message. We'll update the `appearance` component with some _very_ basic text rendering to support this.
...
<Sandbox
  ...
  files={{
    ...
    "/styles.css": styles2,
    "/index.ts": indexTs2,
    "/src/engine.ts": engineTs6,
    "/src/renderSystem.ts": renderSystemTs6,
    "/src/inputSystem.ts": inputSystemTs6,
    "/src/momentumSystem.ts": momentumSystemTs6,
    "/src/collisionSystem.ts": collisionSystemTs6,
    "/src/reboundSystem.ts": reboundSystemTs6,
    "/src/zombieSystem.ts": zombieSystemTs6,
    "/src/lurchSystem.ts": lurchSystemTs6,
    "/src/gameOverSystem.ts": gameOverSystemTs6,
    "/src/utils.ts": utilsTs6,
    "/src/entities.ts": entitiesTs6,
    "/package.json": `{
      "dependencies": {},
        "devDependencies": {
          "typescript": "^4.0.0"
...
import { zombieSystem } from "./sandpackFiles/6/src/zombieSystem";
import { gameOverSystem } from "./sandpackFiles/6/src/gameOverSystem";
import { lurchSystem } from "./sandpackFiles/6/src/lurchSystem";
