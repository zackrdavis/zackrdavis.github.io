...
The `renderSystem` is an odd duck. It doesn't make changes to components (just renders them), and it knows about non-gameworld things like the DOM (or, depending on our rendering target, the canvas API or even WebSockets). We'll need another special system like this to listen for keyboard events.

Because they need to save some long-lived information about the setup, and I want to keep this "impure" logic isolated, I'm going to write them both as classes that we initialize before starting the engine.
