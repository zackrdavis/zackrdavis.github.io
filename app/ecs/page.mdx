...
import forEntsWithTs3 from "!!raw-loader!./sandpackFiles/3/src/forEntsWith.ts";
...
import zombieSystemTs5 from "!!raw-loader!./sandpackFiles/5/src/zombieSystem.ts";
...
One thing missing from my implementation is a way for systems to efficiently query for entities they should act on, a basic affordance of any real engine. I never encountered any performance issues at this tiny scale, so I couldn't bring myself to obscure the beautiful simplicity of the core loops.
...
The `renderSystem` is an odd duck. It knows about non-gameworld things like the DOM and the canvas API. Similarly, the `inputSystem` below has many lines of code for translating keyboard events to player intent. As long as platform details are isolated to these systems, we should have an easy time writing tests for the other systems, and even porting the game to another platform like a Node server.

The `inputSystem` only looks at entities with `velocity` and `playerControlled`, and updates their velocity based on keypresses, affecting them a bit like a strong wind. I've also updated the `momentumSystem` to look at the `friction` component, which will slow down entities over time.
...
Already fun! Students of _this_ universe might notice that all of these systems treat the velocity's X and Y aspect independently, so a diagonal movement will be faster than a cardinal one, and friction might bring an entity to a stop on one axis before the other, imparting a curved path. The physics feel surprisingly nice despite this, so I'm calling it good enough.

Now it's time to add collision-detection, which will be the basis of all interaction between entities. On my first pass I thought collision and rebound would be a single system, but what if we want game conditions triggered by an invisible box? Discovering collisions should be separate from reacting to them.
...
We'll have to compare edges between every collidable entity, so I've written a helper function, `forEntsAndPeersWith`, to make this nested looping readable and type-safe. It also provides a filtered list of collidable peers for each entity.
...
<Sandbox
  ...
  files={{
    ...
    "/src/forEntsWith.ts": forEntsWithTs3,
...
<Sandbox
  ...
  files={{
    ...
    "/src/zombieSystem.ts": zombieSystemTs5,
...
import { zombieSystem } from "./sandpackFiles/5/src/zombieSystem";
import { gameOverSystem } from "./sandpackFiles/5/src/gameOverSystem";
import { lurchSystem } from "./sandpackFiles/5/src/lurchSystem";
