...
The `renderSystem` is special. It knows about non-gameworld things like the DOM, or the canvas API, or maybe WebSockets if we're running the engine on a server. Another impure system will be the `inputSystem`. We want to isolate these systems with side effects and knowledge of the outside world.

We'll upgrade our display system to target an HTML canvas, and we'll accept input from the keyboard. We'll also add `appearance` and `location` components to tell the `renderSystem` how to draw each entity.
